...
function dhcpd (opts) {
  ...
  self.save_lease       = opts.store.save_lease;
  self.get_lease        = opts.store.get_lease;
  self.get_lease_by_ip  = opts.store.get_lease_by_ip;
  self.get_next_ip      = opts.store.get_next_ip;
  self.remove_lease     = opts.store.remove_lease;

  self.s.bind(67, opts.host, function() {
    self.s.setMulticastTTL(255);
    try {
      self.s.addMembership('239.255.255.249', self.host);
    } catch (error) {
      if(error.code == 'ENODEV') {
        throw new Error ("Can't listen on "+self.host+" : no such device");
      } else {
        throw error;
      }
    }
